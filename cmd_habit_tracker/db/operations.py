...
from cmd_habit_tracker.config.settings import DatabaseConfig
...
class DatabaseOperations:
    ...
    def execute_query(self, query, params=''):
        ...
        result = None
        try:
            with sqlite3.connect(self.config.path) as connection:
    ...
    def get_habit_by_id(self, id: int) -> Optional[Habit]:
        """
        @Parameters:
        ...
        @ return value:
            ...
            if not found - None 
        """
        results = execute_query(queries.get_habit_by_id_query(), {'id': id})
        if len(results) == 0:
            return None
    ...
    def get_habit_by_title(self, habit_title: str) -> Habit:
        """
        return value:
            an object of Habit class
        """
    ...
    def convert_tracker_query_result_to_objects(self, results):
        records = [Record() for _ in results]
        [record.set_record_values(res) for record, res in zip(records, results)]
        return records

    def get_all_track_info_of_date(self, date: str):
    ...
    def insert_tracking_info_for_a_specific_date(self, records: list):
        """
    ...
    def get_tracked_info_by_month(self, year: int, month: int):
        """
        if not working try:
...
def get_habit(id: int):
    ...
    @ return value:
        ????? 
    """
    return execute_query(queries.get_habit_by_id_query(), {'id': id})

def get_habit_by_title(habit_title: str) -> Habit:
    """
    return value:
        an object of Habit class
    """
